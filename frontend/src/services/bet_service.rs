...
pub async fn make_bet(player_email: &str, match_id: &u32, team_score: &u8, faced_team_score: &u8) -> Result<Response, reqwest::Error> {
    ...
    let req_builder = client.post("http://localhost:8080/bets");
...
pub async fn edit_bet(player_email: &str, match_id: &u32, team_score: &u8, faced_team_score: &u8) -> Result<Response, reqwest::Error> {
    let token = web_sys::window().unwrap().local_storage().unwrap().unwrap().get_item("token").unwrap();
    let mut map = HashMap::new();
    map.insert("playerEmail", player_email.to_string());
    map.insert("matchId", match_id.to_string());
    ...
    let req_builder = client.put("http://localhost:8080/bets");
...
pub async fn get_bets_by_player_by_match_id(player_email: String, match_id: String) -> Result<Bet, reqwest::Error> {
    let token = web_sys::window().unwrap().local_storage().unwrap().unwrap().get_item("token").unwrap();
    let client = Client::new();

    let req_builder = client.get(format!("http://localhost:8080/bets/{}/{}", player_email, match_id));
